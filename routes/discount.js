...
router.get("/:id", async (req, res) => {
  ...
  try {
    getDis_data = await pool.query(
      "SELECT p.id, p.category, p.color, p.shopname, p.shoppicture, p.description, p.stock, p.product_image, p.productname, p.price, p.rating FROM product_data p INNER JOIN discount_product d ON d.discountid = p.idp WHERE p.id = $1",[id]
    );
    res.status(200).json({ message: "All resources", data: getDis_data.rows });
  } catch (error) {}
});
